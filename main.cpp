...
void save_cell(Cell *cell, const string& file_name){

  ofstream f(file_name, ios::app); 

  f << "("<<cell->m_x<<","<<cell->m_y<<")"<<cell->m_nb_neighb;

  for(int i = 0 ; i < cell->m_nb_neighb ; i++)
  {

    f << "("<<cell->m_neighb[i]->m_x<<","<<cell->m_neighb[i]->m_y<<")";
  ...
  for(int i = 0 ; i < cell->m_nb_neighb ; i++)
  {
    if(!cell->m_neighb[i]->m_written)

      save_cell(cell->m_neighb[i], file_name);
...
void save_maze(const Maze& maze, const string& file_name){
   ...
   ofstream f(file_name, ios::app); 
   f<<" # Start " <<endl;
   f << "("<<maze.c0->m_x<<","<<maze.c0->m_y<<")"<<endl;
    f<<" # End " <<endl;
   f << "("<<maze.cf->m_x<<","<<maze.cf->m_y<<")"<<endl;
   f<<" # Cells " <<endl;

   f.close();

  save_cell(maze.c0, file_name);
...
int main()
{
  ...
  string filename = "/home/martinho/Documents/cpp_03_tp_correction/file.txt";
  ofstream f(filename); 
